...
def validaciondealpha():
    ...
    if(Euclides(a,n)==1):
        ...
        messagebox.showinfo("Alpha","El valor de Alpha es valido\n\n" + 
                            f"El inverso multiplicativo de alfa es: {inverso}\n\n"+
                            f"La función de cifrado es:\nC={a}p+{b} mod {n}\n"+
                            f"La función de decifrado es:\np={inverso}[C+(-{b})] mod {n}")
    else:
        messagebox.showinfo("Alpha","El valor de Alpha no es valido. Introduzca un nuevo valor para Alpha")
...
def validaciondebeta():
    try: #Validacion de entradas
        ...
        return
    if(b>0 and b<=n):
        messagebox.showinfo("Beta","Beta es valido")
    else:
        messagebox.showinfo("Beta","Beta no esta dentro del conjunto (0,n]")
...
#Si alfa es valido lo calcula directamente en su validacion
